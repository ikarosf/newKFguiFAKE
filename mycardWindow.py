...
class Ui_mycardForm(object):
    ...
    def makeMyCard(self):
        ...
        card = myCard(halo, character, level, attrSet, sklSlot, skillSet, equipSet)
    ...
    def myCardListUpdate(self):
        myCardList = global_env.myCardList
    ...
    def saveMyCard(self):
        ...
        if ok and text:
            if text in global_env.myCardList.keys():
                QMessageBox.critical(self, "错误", "保存失败，与已有配置重名", QMessageBox.Yes)
                return
            global_env.myCardList[text] = newCard
